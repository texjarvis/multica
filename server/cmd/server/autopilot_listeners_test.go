...
func TestAutopilotRunOnlyTaskTerminalEventsUpdateRun(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "failed",
			finalize: func(task db.AgentTaskQueue) {
				if _, err := taskSvc.FailTask(ctx, task.ID, "boom", "", "", "agent_error", false, nil); err != nil {
...
func TestAutopilotCreateIssueTaskNoProgressFailureUpdatesRun(t *testing.T) {
	...
	if _, err := f.taskSvc.FailTask(ctx, f.taskID, errMsg, "", "", "codex_semantic_inactivity", false, nil); err != nil {
		t.Fatalf("FailTask: %v", err)
	}
...
func TestAutopilotCreateIssueTaskAgentErrorFailureUpdatesRun(t *testing.T) {
	...
	if _, err := f.taskSvc.FailTask(ctx, f.taskID, errMsg, "", "", "agent_error", false, nil); err != nil {
		t.Fatalf("FailTask: %v", err)
	}
...
func TestAutopilotCreateIssueTaskRetryPendingKeepsRunOpen(t *testing.T) {
	...
	if _, err := f.taskSvc.FailTask(ctx, f.taskID, "runtime went offline", "", "", "timeout", false, nil); err != nil {
		t.Fatalf("FailTask: %v", err)
	}
