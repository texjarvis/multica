...
func TestCreateWorktreeMigratesLinkedWorktreeToIsolatedMetadata(t *testing.T) {
	...
	// Fabricate a genuine legacy (pre-fix) checkout: a linked `git worktree
	// add` whose .git is a file pointing back into the shared bare cache,
	// exactly what an older daemon would have left behind. A plain
	// CreateWorktree call no longer produces this shape (its default mode is
	// now the task-local .git directory from runWorktreeAdd), so this has to
	// be built directly with git, same as TestCreateWorktreeMigratesLegacyLinkedWorktree.
	workDir := t.TempDir()
	...
	if !isLegacyGitWorktree(legacyPath) {
		t.Fatal("test setup did not create a legacy linked worktree")
	}
	...
	if isolated.Path != legacyPath {
		t.Fatalf("migrated checkout path = %q, want %q", isolated.Path, legacyPath)
	}
	...
	if strings.Contains(string(out), "worktree "+legacyPath+"\n") {
		t.Fatalf("shared cache retained migrated worktree admin entry:\n%s", out)
	}
}

func TestCreateWorktreeMigratesPlainCheckoutToIsolatedMetadata(t *testing.T) {
	t.Parallel()
	sourceRepo := createTestRepo(t)
	cache := New(t.TempDir(), testLogger())
	if err := cache.Sync("ws-1", []RepoInfo{{URL: sourceRepo}}); err != nil {
	...
	// First checkout goes through the non-isolated default path: a plain
	// task-local .git directory (runWorktreeAdd) with no isolated-checkout
	// marker set.
	plain, err := cache.CreateWorktree(params)
	if err != nil {
		t.Fatalf("plain CreateWorktree failed: %v", err)
	}
	if isIsolatedCheckout(plain.Path) {
		t.Fatal("test setup: plain checkout should not already be marked isolated")
	}
	if info, err := os.Stat(filepath.Join(plain.Path, ".git")); err != nil || !info.IsDir() {
		t.Fatalf("test setup did not create a plain .git directory: info=%v err=%v", info, err)
	}

	// A later task on the same reused workdir requests isolated metadata —
	// e.g. a provider transition. This must migrate the plain checkout in
	// place rather than failing.
	params.TaskID = "44444444-4444-4444-4444-444444444444"
	params.IsolatedGitMetadata = true
	isolated, err := cache.CreateWorktree(params)
	if err != nil {
		t.Fatalf("isolated CreateWorktree failed: %v", err)
	}
	if isolated.Path != plain.Path {
		t.Fatalf("migrated checkout path = %q, want %q", isolated.Path, plain.Path)
	}
	if !isIsolatedCheckout(isolated.Path) {
		t.Fatal("plain checkout was not migrated to isolated metadata")
	}
}

func TestCreateWorktreeRejectsIsolatedMigrationAcrossBasenameCollision(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	repoADir := filepath.Join(root, "org-a", "shared-name")
	repoBDir := filepath.Join(root, "org-b", "shared-name")
	if err := os.MkdirAll(filepath.Dir(repoADir), 0o755); err != nil {
		t.Fatalf("mkdir repoA parent: %v", err)
	}
	if err := os.MkdirAll(filepath.Dir(repoBDir), 0o755); err != nil {
		t.Fatalf("mkdir repoB parent: %v", err)
	}
	// Different repos, same final path segment: repoNameFromURL only keeps
	// that segment, so both resolve to the same worktreePath under a shared
	// WorkDir — the collision this test guards against.
	repoA := createTestRepoAt(t, repoADir)
	repoB := createTestRepoAt(t, repoBDir)
	if repoNameFromURL(repoA) != repoNameFromURL(repoB) {
		t.Fatalf("test setup: repos must share a basename, got %q and %q", repoNameFromURL(repoA), repoNameFromURL(repoB))
	}
	...
	workDir := t.TempDir()
	checkoutA, err := cache.CreateWorktree(WorktreeParams{
		WorkspaceID: "ws-1",
		RepoURL:     repoA,
		WorkDir:     workDir,
		AgentName:   "Linux Codex",
	...
	if err != nil {
		t.Fatalf("checkout repo A failed: %v", err)
	}
	marker := filepath.Join(checkoutA.Path, "repo-a-marker.txt")
	if err := os.WriteFile(marker, []byte("repo A work"), 0o644); err != nil {
		t.Fatalf("write marker: %v", err)
	}

	// Same WorkDir + colliding basename means repo B resolves to the exact
	// same worktreePath as repo A's checkout above. Requesting isolated
	// metadata for repo B here must fail rather than silently destroying
	// repo A's checkout.
	if _, err := cache.CreateWorktree(WorktreeParams{
		...
		t.Fatal("expected checkout of colliding basename repo to fail, got nil error")
	}

	if _, statErr := os.Stat(marker); statErr != nil {
		t.Fatalf("repo A's checkout was destroyed by the colliding repo B request: %v", statErr)
	}
}
