...
Runtimes UI는 명령 문자열을 argv로 파싱합니다. CLI는 고정 argv를 JSON 문자열 배열로 받습니다.

공개 runtime profile 응답은 임의의 실행 인수에 자격 증명이 포함될 수 있으므로 `fixed_args` 값을 노출하지 않습니다. 빈 플레이스홀더와 `fixed_args_count` / `fixed_args_redacted`만 반환하며, 데몬 profile 동기화 엔드포인트는 런타임 실행에 필요한 원본 argv를 그대로 받습니다. profile 편집은 숨겨진 인수를 기본적으로 보존합니다. API 업데이트는 새 비어 있지 않은 목록과 `fixed_args_intent: "replace"`를, 빈 목록과 `fixed_args_intent: "clear"`를 함께 보냅니다. UI와 CLI는 이를 명시적인 **인수 교체**와 **인수 지우기** 동작으로 제공합니다. 빈 구형 응답 플레이스홀더 재전송은 숨김 인수를 보존하며, 숨김 인수가 있을 때 모호한 비어 있지 않은 구형 업데이트는 거부됩니다.

CLI로 쓸 때는 `--fixed-args-file <0600-json>` 또는 `--fixed-args-stdin`을 권장합니다. 인라인 `--fixed-args <json>`은 셸 기록과 프로세스 목록에 자격 증명을 노출할 수 있습니다. 세 입력 채널은 상호 배타적이고, 빈 file/stdin 입력은 fail-closed로 거부되며, update 교체는 비어 있지 않아야 합니다. 지우기는 `--clear-fixed-args`만 사용할 수 있습니다.
