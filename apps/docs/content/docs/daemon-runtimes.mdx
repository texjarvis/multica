...
multica runtime profile create --display-name "Composer" --protocol-family codex --command-name agent --fixed-args-file ./composer-args.json
multica runtime profile update <profile-id> --fixed-args-stdin
multica runtime profile update <profile-id> --clear-fixed-args
...
shell behavior. The Runtimes UI parses command text into argv. The CLI accepts
fixed argv as a JSON string array.

Public runtime-profile responses never expose `fixed_args` values because arbitrary launch arguments can contain credentials. They return an empty placeholder plus `fixed_args_count` / `fixed_args_redacted`; the daemon profile-sync endpoint still receives the raw argv needed to launch the runtime. Editing a profile preserves hidden arguments by default. API updates pair a fresh non-empty list with `fixed_args_intent: "replace"` or an empty list with `fixed_args_intent: "clear"`; the UI and CLI expose those as explicit **Replace arguments** and **Clear arguments** actions. An empty legacy response replay preserves hidden arguments, while an ambiguous non-empty legacy update is rejected once hidden arguments exist.

For CLI writes, prefer `--fixed-args-file <0600-json>` or `--fixed-args-stdin`. Inline `--fixed-args <json>` can expose credentials in shell history and process listings. The three channels are mutually exclusive, empty file/stdin input fails closed, update replacement must be non-empty, and `--clear-fixed-args` is the only clear action.
