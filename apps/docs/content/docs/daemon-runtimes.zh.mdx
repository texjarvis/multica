...
Runtimes UI 会把命令文本解析成 argv；CLI 则以 JSON 字符串数组接收固定 argv。

公开的 runtime profile 响应不会暴露 `fixed_args` 值，因为任意启动参数都可能包含凭据。响应只返回空占位以及 `fixed_args_count` / `fixed_args_redacted`；守护进程的 profile 同步端点仍会收到启动运行时所需的原始 argv。编辑 profile 时默认保留隐藏参数。API 更新必须把新的非空列表与 `fixed_args_intent: "replace"` 配对，或把空列表与 `fixed_args_intent: "clear"` 配对；UI 和 CLI 将它们提供为明确的**替换参数**和**清空参数**操作。回放旧响应中的空占位会保留隐藏参数；已有隐藏参数时，非空但无 intent 的旧式模糊更新会被拒绝。

通过 CLI 写入时，优先使用 `--fixed-args-file <0600-json>` 或 `--fixed-args-stdin`。内联 `--fixed-args <json>` 可能在 shell 历史和进程列表中暴露凭据。三种输入通道互斥；空的 file/stdin 输入会 fail-closed；update 的替换列表必须非空；只有 `--clear-fixed-args` 能清空参数。
